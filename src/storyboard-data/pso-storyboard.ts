...
export const masterStoryboard: Storyboard = {
  ...
  steps: [
    ...
    },
    {
      ...
      notes:
        "这一页重新回到单独终页：上半只保留《逍遥游》收束，下半不再使用左右两个推荐卡，而是改成左二中二维码右二的三栏入口；中间放主仓库直角二维码，左右四个条目继续保留轻简介，其中右列改成右对齐并与左列一起对称内收。",
      focusTarget: "Further Reading",
      timingHint:
        "延续前一页的停顿感，整页淡入即可；上半 quote 先成立，下半左右两列与中间二维码再稳稳落位。",
      intro:
        "最后再把引句和推荐入口重新拆回单独终页。",
      manuscript:
        "第三十三页重新承担全场最后一页。上半保留《庄子·逍遥游》的引句：`今子有大树，患其无用，何不树之于无何有之乡，广莫之野，彷徨乎无为其侧，逍遥乎寝卧其下。不夭斤斧，物无害者，无所可用，安所困苦哉！` 下半这次不再保留左右两个大框，也不再写 `书与视频`、`推荐游戏` 这种分类标题，而是直接改成左二中二维码右二的三栏入口。左边放 `《银河帝国》` 和 `人类高质量思政课`，右边放 `星际拓荒（Outer Wilds）` 和 `Type Help`，每个条目都继续保留一行轻简介；其中右列统一右对齐，左右两列再对称向中间收一点，让二维码区更像一个稳定出口。中间则放主仓库 `https://github.com/ZJUZWT/MT-PSO-Talk` 的直角二维码，观众用微信扫一扫就可以直接跳到网页。原来的 `《反杜林论》` 与 `重读资本论` 这次继续保持删掉，不再拉长推荐清单。这样最后一页既保留了一个更松弛的收束，也把“继续往下看”真正收成一个更干净的出口。",
      focusColorKey: "shared",
    },
  ],
};
