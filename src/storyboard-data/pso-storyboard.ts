...
export const masterStoryboard: Storyboard = {
  ...
  steps: [
    ...
    },
    {
      ...
      keyPoints: [
        "左半明确读成 `分集合`：判据是 `对象可聚类 / 需求具上下文 / 允许分组裁剪`，熟悉类比是 `画质分档 / 功能分档`；PSO 对应是 `Game UsageMask / Compile UsageMask`（自研扩展） / `Per-Map Compile`。",
        "右半明确读成 `并行化`：判据是 `任务可拆分 / 依赖稀疏 / 结果可汇总`，不再画 queue / worker 拓扑，而是只保留 `SIMD / Thread / GPU` 三层并行层级，并补一句 `SIMD -> DOP、ECS / GPU -> ATT`。",
        "这页的逻辑从“两个 UE 技巧”改成“编译过慢时的两种常见方法”：`分集合` 决定做什么，`并行化` 决定怎么更快做完。",
      ],
      apiHighlights: [
        "UE 策略：Game UsageMask / Compile UsageMask（自研扩展）/ Per-Map Compile",
        "通用并行层级：SIMD / Thread / GPU（含 DOP、ECS、ATT）",
      ],
      notes:
        "这一页不再试图用很多盒子讲清两个 UE 内部实现，而是直接讲两种高层方法：左边 `分集合`，右边 `并行化`。`UsageMask` 只是分集合的一种实现，其中 `Game UsageMask` 是 UE 原生字段，`Compile UsageMask` 是我们在其之上自研的扩展机制（预编译侧允许直接挂 `B+C` 这种待命中的集合，不替代 UE 字段）。右侧 `SIMD / Thread / GPU` 是通用并行层级，并非特指 PSO 编译本身；这次在层级表里再补两个贴近工程实现的提示：`SIMD` 后面挂 `DOP、ECS`，强调单核里的数据并行与组织方式；`GPU` 后面挂 `ATT`，把它读成一次从 CPU 到 GPU 的执行位置转移。",
      ...
      manuscript:
        "第二十六页现在也不再讲两个孤立技巧，而是讲编译过慢时的两种更一般的方法。左边是 `分集合`：当对象本身可以聚类，需求又带着很强的上下文时，就不必把所有东西一把梭都做完，而是可以先按集合切开。这里给的熟悉类比扩成 `画质分档 / 功能分档`，强调分集合不只是在内容分地图，也可以在能力层面做裁剪；而 PSO 这边的实现分两层：`Game UsageMask` 是 UE 原生字段，由运行时依据当前场景 / 地图置位；`Compile UsageMask` 是我们自研的扩展机制，允许预编译侧直接挂载一组待命中的集合，不替代 UE 字段。所以画面继续坚持先从事件讲起：当前玩家在地图 A，这时点击下载 B+C，我们就希望立刻把 B+C 作为下一轮待编译集合挂进去。于是同一时刻会出现两个视角：站在当前游戏视角，`Game UsageMask = A`；站在编译调度视角，`Compile UsageMask = B+C`。这条路解决的是“这一轮到底做哪一部分”。右边是 `并行化`：当任务可以拆开，彼此依赖不强，最后结果又能汇总时，就可以把同样的集合更快做完。这里不再硬找一个不太贴切的日常类比，而是直接给出工程上更常见的三层并行层级：`SIMD / Thread / GPU`。这次层级表里再补两个贴近实现的提示：`SIMD -> DOP、ECS`，`GPU -> ATT`。其中 `ATT` 在这里可以直接读成一次从 CPU 到 GPU 的执行位置转移。所以这一页最后想让观众记住的不是某个 UE 术语，而是一句更高层的话：编译过慢时，先看能不能 `分集合`，再看能不能 `并行化`。",
      focusColorKey: "shared",
    },
    {
      id: "page_27",
