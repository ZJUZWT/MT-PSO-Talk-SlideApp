...
describe("page_19 vertical relief harness goals", () => {
  it("lands the simplified disk route on the disk box boundary without piercing", () => {
    const artifact = buildGeometryReviewArtifact(page19R1Sketch);
    const diskRoute = artifact.edgeRouteMetrics.find(
      (metric) => metric.edgeId === "gfx-to-disk",
    );
    const diskAnchor = artifact.edgeAnchorMetrics.find(
      (metric) => metric.edgeId === "gfx-to-disk",
    );

    expect(artifact.metrics.nodePierceCount).toBe(0);
    expect(diskRoute).toMatchObject({
      edgeId: "gfx-to-disk",
      nodePierceCount: 0,
      badEndpointCount: 0,
    });
    expect(diskAnchor).toMatchObject({
      edgeId: "gfx-to-disk",
      toNodeId: "disk-group",
      toSide: "left",
      toOffsetPx: 0,
  ...
  it("keeps the slim three-column anchors centered while holding the top-row layout in range", () => {
    const artifact = buildGeometryReviewArtifact(page19R1Sketch);
    ...
    expect(artifact.metrics.minNodeGap).toBeGreaterThanOrEqual(20);
    ...
    expect(stableNode?.height ?? 0).toBeLessThanOrEqual(240);
    expect(gfxNode?.height ?? 0).toBeLessThanOrEqual(240);
    expect(diskNode?.height ?? 0).toBeLessThanOrEqual(240);
